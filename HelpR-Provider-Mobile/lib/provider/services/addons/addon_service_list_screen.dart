...
class _AddonServiceListScreenState extends State<AddonServiceListScreen> {
  ...
  Future<void> confirmationDialog({required ServiceAddon addonServiceData}) async {
    showConfirmDialogCustom(
      context,
      title: '${languages.areYouSureWantToDeleteThe} ${addonServiceData.name.validate()} ${languages.addOns}?',
                ...
                itemBuilder: (BuildContext context, index) {
                  ...
                  return Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        CachedImageWidget(
                          url: data.serviceAddonImage.validate().isNotEmpty ? data.serviceAddonImage.validate() : '',
                          height: 70,
                          fit: BoxFit.cover,
                        ...
                        Column(
                          ...
                          children: [
                            4.height,
                            Marquee(child: Text(data.name.validate().capitalizeFirstLetter(), style: boldTextStyle())),
                            4.height,
                            if (data.serviceName.isNotEmpty) Text('${data.serviceName.validate()}', style: secondaryTextStyle()).paddingBottom(4),
                            PriceWidget(
                        ...
                        PopupMenuButton(
                          ...
                          onSelected: (selection) async {
                            if (selection == 1) {
                              bool? res = await AddAddonServiceScreen(addonServiceData: data).launch(context);
