...
import 'package:handyman_provider_flutter/provider/services/components/service_pin_address.dart';
...
class _AddServiceComponentState extends State<AddServiceComponent> {
  ...
  // Method to handle location pinning
  Future<void> _pinLocation() async {
    var result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => PinLocationScreen()), // Navigate to the PinLocationScreen
    );

    if (result != null) {
      // If a location is selected, update the address and latitude/longitude
      setState(() {
        addressNameCont.text = result['address'];
        // You can save the latitude and longitude if needed in the request here
      });
    }
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    AppButton(
                      ...
                      onTap: _pinLocation, // Calls the method to pin location
                    ),
                    24.height,
                    AppButton(
